...
import {
  Shield, Users, CheckCircle, UserX, UserPlus,
  UserCheck, RefreshCw, Crown, Clock, AlertTriangle,
  Activity, Key, Globe, LogIn
} from 'lucide-react'
import {
  getAdminStats, getAdminUsers, getAdminLogins, suspendUser, unsuspendUser
} from '../services/api'
...
function StatusPill({ is_active }) {
  return is_active ? (
    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold"
      style={{ background: 'rgba(0,229,153,0.12)', color: '#00e599', border: '1px solid rgba(0,229,153,0.3)' }}>
      Active
    </span>
  ) : (
    <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold"
      style={{ background: 'rgba(131,142,158,0.12)', color: '#838e9e', border: '1px solid rgba(131,142,158,0.3)' }}>
      Suspended
    </span>
  )
}
  ...
  return (
    <div className="panel flex items-center gap-3 py-3">
      ...
      <div>
        ...
        {sub && <p className="text-[10px] mt-0.5" style={{ color: 'var(--accent)' }}>{sub}</p>}
      </div>
    </div>
  )
}
  ...
  return (
    <div className="p-6 space-y-5">
      <PageHeader
        ...
        subtitle="System overview, registered users, and live authentication audits"
        actions={
      ...
      {msg.text && (
        <div className="px-4 py-2.5 rounded text-sm"
          style={msg.type === 'success'
            ? { background: 'rgba(0,229,153,0.08)', border: '1px solid rgba(0,229,153,0.3)', color: '#00e599' }
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard
            ...
            color="var(--accent)"
            sub={stats.new_users_today ? `+${stats.new_users_today} registered today` : null}
          />
          <StatCard
            icon={UserPlus}
            label="New This Week"
      ...
      {/* Navigation Tabs (Users vs Login Activity) */}
      <div className="flex items-center gap-3 border-b pb-1" style={{ borderColor: 'var(--border)' }}>
        <button
          onClick={() => setActiveTab('users')}
          className={`flex items-center gap-2 px-3 py-2 text-xs font-semibold rounded-t transition-all ${
            activeTab === 'users'
        ...
        >
          ...
          Login &amp; Registration Logs ({logins.length})
        </button>
      </div>
      ...
      {activeTab === 'users' && (
        <div className="space-y-3">
          {/* Filter Sub-tabs */}
          <div className="panel py-2 px-3 flex items-center gap-2 flex-wrap">
            <span className="text-xs font-bold uppercase tracking-wider mr-2" style={{ color: 'var(--muted)' }}>
              Filter:
            </span>
            {[
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ borderBottom: '2px solid var(--border)', background: 'var(--surface2)' }}>
                      {['#', 'User', 'Email', 'Role', 'Status', 'Alerts', 'Registered Date', 'Actions'].map(h => (
      ...
      {/* ── TAB 2: Login & Registration Activity ── */}
      {activeTab === 'logins' && (
        <div className="panel p-0 overflow-hidden">
          {loading ? (
            <div className="flex justify-center py-16"><Spinner size="lg" /></div>
            ...
            <div className="text-center py-12">
              <Clock className="w-8 h-8 mx-auto mb-2" style={{ color: 'var(--muted)' }} />
              <p className="text-sm" style={{ color: 'var(--muted)' }}>No recent login logs recorded</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr style={{ borderBottom: '2px solid var(--border)', background: 'var(--surface2)' }}>
                    {['#', 'User / Email', 'Event', 'Status', 'Timestamp', 'IP Address'].map(h => (
                      <th key={h} className="text-left py-3 px-4 whitespace-nowrap text-xs font-bold uppercase tracking-wide"
                        style={{ color: 'var(--muted)' }}>
                        {h}
                      </th>
                    ...
                    <tr key={entry.id || idx}
                      ...
                      <td className="py-3 px-4">
                        {entry.status === 'Success' ? (
                          ...
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold"
                            style={{ background: 'rgba(255,34,68,0.12)', color: '#ff2244', border: '1px solid rgba(255,34,68,0.3)' }}>
                            Failed
                          </span>
                        ) : (
                          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold"
      ...
      <div className="flex items-start gap-3 px-4 py-3 rounded"
        ...
        <p className="text-xs leading-relaxed" style={{ color: 'var(--muted)' }}>
          <span className="font-semibold" style={{ color: 'var(--text)' }}>Confidentiality &amp; Privacy: </span>
          Admin sees system metadata (account name, email, registration date, login timestamps, and account status).
          Alert content, raw payloads, and AI investigation reports remain strictly private to alert owners.
        </p>
      </div>
    </div>
  )
