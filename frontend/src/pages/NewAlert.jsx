...
import { Upload, Play, ChevronRight, ArrowLeft, FileText, Zap } from 'lucide-react'
import { submitAlert, uploadAlertFile, getDemoScenarios, getDemoRaw, loadDemoScenario } from '../services/api'
...
export default function NewAlert() {
  ...
  return (
    <div className="p-6 max-w-3xl">
      ...
      {/* Mode tabs */}
      <div className="flex gap-1 p-1 bg-cyber-surface rounded-lg border border-cyber-border mb-6 w-fit">
        {[
      ...
      {mode === 'paste' && (
        <form onSubmit={handlePaste} className="card space-y-4">
          ...
          <button
            type="submit"
            disabled={submitting || !rawAlert.trim()}
          ...
          <div>
            ...
            >
              ...
              {file ? (
                ...
                <p className="text-sm text-cyber-muted">Click to select file, or drag and drop</p>
              )}
              <p className="text-xs text-cyber-muted mt-1">.log, .txt</p>
            </div>
            <input
              ref={fileRef}
              type="file"
            ...
            <div key={s.id} className="card flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className={`text-xs font-mono font-semibold ${RISK_HINT_COLORS[s.risk_level_hint] ?? ''}`}>
                    {s.risk_level_hint}
                  </span>
                </div>
                <p className="text-xs text-cyber-muted">{s.description}</p>
                <p className="text-xs text-cyber-muted mt-0.5">Source: <span className="uppercase">{s.source}</span></p>
              </div>
