...
from app.services.alert_service import (
    create_alert, get_alert, list_alerts, save_analysis, update_alert_status,
)
...
def _ser(alert):
    ...
    if getattr(alert, "notes", None):
        data["notes"] = [{"id": n.id, "alert_id": n.alert_id, "note": n.note,
...
async def upload_alert_file(
    ...
    alert = create_alert(db, AlertCreate(raw_alert=raw, source=source))
    return _ser(alert)
...
def get_alerts(
    ...
    total, alerts = list_alerts(db, skip=skip, limit=limit, risk_level=risk_level,
    ...
    return {"total": total, "items": [
        {"id": a.id, "timestamp": a.timestamp.isoformat() if a.timestamp else None,
         "source": a.source, "alert_type": a.alert_type, "source_ip": a.source_ip,
...
async def analyze_alert(alert_id: int, db: Session = Depends(get_db)):
    ...
    # AI call is async (HTTP to Ollama); run it
    ...
    return {"id": analysis.id, "alert_id": analysis.alert_id,
...
def change_status(alert_id: int, payload: AlertStatusUpdate, db: Session = Depends(get_db)):
    alert = update_alert_status(db, alert_id, payload)
    if not alert:
        raise HTTPException(404, "Alert not found")
