"""Alert business logic — all queries scoped to owner_id."""
...
def create_alert(db: Session, payload: AlertCreate, owner_id: int) -> Alert:
...
def get_alert(db: Session, alert_id: int, owner_id: int) -> Alert | None:
    return (
        db.query(Alert)
        .options(selectinload(Alert.analysis), selectinload(Alert.notes))
        .filter(Alert.id == alert_id, Alert.owner_id == owner_id)
        .first()
    )


def list_alerts(
    db: Session, owner_id: int, *, skip=0, limit=50,
    risk_level=None, alert_type=None, source_ip=None,
    status=None, search=None, date_from=None, date_to=None,
):
...
def update_alert_status(
    db: Session, alert_id: int, owner_id: int, payload: AlertStatusUpdate
) -> Alert | None:
    alert = db.query(Alert).filter(
        Alert.id == alert_id, Alert.owner_id == owner_id
    ).first()
    if not alert:
        return None
...
def save_analysis(
    db: Session, alert_id: int, owner_id: int, *,
    summary, threat_interpretation, evidence,
    risk_explanation, recommendations,
    ai_model="rule-based", is_ai_generated=False,
) -> Analysis:
    # Verify ownership
    alert = db.query(Alert).filter(
        Alert.id == alert_id, Alert.owner_id == owner_id
...
def get_dashboard_stats(db: Session, owner_id: int) -> dict[str, Any]:
    q = db.query(Alert).filter(Alert.owner_id == owner_id)
    total = q.count()
    levels = {l: q.filter(Alert.risk_level == l).count()
              for l in ("CRITICAL", "HIGH", "MEDIUM", "LOW")}
    statuses = {s: q.filter(Alert.status == s).count()
                for s in ("new", "investigating")}
    top_types = [
        {"alert_type": r[0], "count": r[1]}
        for r in db.query(Alert.alert_type, func.count(Alert.id).label("c"))
        .filter(Alert.owner_id == owner_id)
        .group_by(Alert.alert_type)
        .order_by(func.count(Alert.id).desc()).limit(5)
    ]
    top_ips = [
        {"source_ip": r[0], "count": r[1]}
        for r in db.query(Alert.source_ip, func.count(Alert.id).label("c"))
        .filter(Alert.owner_id == owner_id, Alert.source_ip.isnot(None))
        .group_by(Alert.source_ip)
        .order_by(func.count(Alert.id).desc()).limit(5)
    ]
    return {
        "total_alerts": total,
        "critical_alerts": levels["CRITICAL"],
        "high_alerts": levels["HIGH"],
        "medium_alerts": levels["MEDIUM"],
        "low_alerts": levels["LOW"],
        "new_alerts": statuses["new"],
